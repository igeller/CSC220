...
public class SortTest<E extends Comparable<E>> {
  ...
  public static void tests (int n) {
    ...
    Integer [] array1 = new Integer[]{ 3, 1, 4, 1, 5, 9, 2, 6};
...
class InsertionSort<E extends Comparable<E>> implements Sorter<E> {
  public void sort (E[] array) {
    for (int n = 0; n < array.length; n++) {
      ...
      //while i>0 && while array[i-1] > data move array[i-1] to array[i] and
      //pop the number you want to move out, compare and shift the larger numbers
      //and then place your number in the new spot and decrement i
    ...
    // compare, getLeft, getRight, and isNull.
	 while( (!isNull(getLeft(index)) && compare(index, getLeft(index))> 0 )|| (!isNull(getRight(index)) && compare(index, getRight(index)) >0) ) {
		...
		/* if(isNull(getRight(index))) {
		 ...
		 //works with above statements but slows time when ( > 0 and the first one is getLeft and the second part is getright below)
		  if(!isNull(getRight(index)) && (compare(getRight(index), getLeft(index)) < 0) ) {
		        // Swap index with right child and set index.
		    			swap(index, getRight(index));
		    			index = getRight(index);
