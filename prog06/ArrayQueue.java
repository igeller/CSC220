...
public class ArrayQueue<E> extends AbstractQueue<E>
 ...
 // private static final int DEFAULT_CAPACITY = 10;
  ...
  private boolean labReallocate = false; 	//set from true to false according to step 8
  ...
  private int nextIndex (int index) {
    if (index + 1 < theItems.length)
      return index + 1;	//if not at end of the array
    else
      return 0;			//if at end of array
  }
	    ...
	    if (labReallocate) {
	      ...
	      // EXERCISE 8
	    	if(front < back)	{   	
	    		...
	    		System.arraycopy(theItems, front, newItems, 0, theItems.length- front);
	    		System.arraycopy(theItems, 0, newItems, theItems.length-front, front);
	    	}
    }
    front = 0;
    back = size - 1;
  ...
  private boolean labIterator = false;			//set from true to false according to exercise 9
